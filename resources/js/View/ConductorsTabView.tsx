      ...
      <Stack gap={2}>
        {
          conductors.children.length > 0
            ? conductors.children.map(el => <ConductorsDisplay
              conductors={el}
              key={conductors.name}/>)
            : conductors.conductors.length > 0
              ? conductors.conductors.map(cond => <ConductorDisplay
                conductor={cond}
...
export const ConductorsTabView: React.FC = () => {
  ...
  return <>
    {conductors.length > 0 ? conductors.map(conductor => <ConductorsDisplay
