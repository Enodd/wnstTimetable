...
@php
    ...
    $tableWidth = $hourColumnWidth + $groupCount * $eventColumnWidth;
@endphp

@foreach ($visibleDays as $dayIndex => $dayName)
    @php $skip = []; @endphp
    <div
        x-show="selectedDay === {{ $dayIndex }}"
        class="timetable-table-wrap"
    >
                ...
                @for ($hour = $startHour; $hour < $endHour; $hour++)
                    @for ($quarter = 0; $quarter < $slotsPerHour; $quarter++)
                        ...
                        <tr class="timetable-slot-row {{ $quarter === 0 ? 'is-hour-start' : '' }}">
                            @if ($quarter === 0)
                                <td rowspan="{{ $slotsPerHour }}" class="timetable-hour-cell timetable-hour-cell--day">
                            ...
                            @for ($groupIndex = 0; $groupIndex < $groupCount; $groupIndex++)
                                @php
                                    ...
                                    if (isset($skip[$groupIndex][$cellId])) continue;
                                ...
                                @if (count($events) > 0)
                                    ...
                                    <td rowspan="{{ $maxDuration }}" class="timetable-event-cell">
                                        <div class="timetable-event-stack {{ $multipleEvents ? 'has-overlap' : '' }}">
                                            @foreach ($events as $event)
                                                @php
                                                    $borderColor = '#' . \helpers\ColorHelper::bgrIntToHex($event['color'] ?? 0);
                                    ...
                                    @php
                                        for ($offset = 1; $offset < $maxDuration; $offset++) {
                                            $skip[$groupIndex][$baseId + $rowIndex + $offset] = true;
                                        }
                                    @endphp
                                @else
